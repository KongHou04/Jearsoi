...
export class ListComponent implements OnInit {
  ...
  @ViewChild('deliverConfirmModal1') deliverConfirmModal1!: TemplateRef<any>;
  @ViewChild('deliverConfirmModal2') deliverConfirmModal2!: TemplateRef<any>;
  @ViewChild('deliverConfirmModal3') deliverConfirmModal3!: TemplateRef<any>;
  ...
  openDeliverModal1(event: Event, order: Order): void {
    event?.stopPropagation();
    this.selectedOrder = order;
    this.modalService.open(this.deliverConfirmModal1, { ariaLabelledBy: 'modal-basic-title' });
  }
  confirmDeliver1(modal: any, callback: (status: number, id?: string) => void): void {
    console.log(this.selectedOrder);
    callback(1, this.selectedOrder?.orderId);
    modal.close();
  }

  openDeliverModal2(event: Event, order: Order): void {
    event?.stopPropagation();
    this.selectedOrder = order;
    this.modalService.open(this.deliverConfirmModal2, { ariaLabelledBy: 'modal-basic-title' });
  }
  confirmDeliver2(modal: any, callback: (status: number, id?: string) => void): void {
    console.log(this.selectedOrder);
    callback(2, this.selectedOrder?.orderId);
    modal.close();
  }

  openDeliverModal3(event: Event, order: Order): void {
    event?.stopPropagation();
    this.selectedOrder = order;
    this.modalService.open(this.deliverConfirmModal3, { ariaLabelledBy: 'modal-basic-title' });
  }
  confirmDeliver3(modal: any, callback: (status: number, id?: string) => void): void {
    console.log(this.selectedOrder);
    callback(3, this.selectedOrder?.orderId);
    modal.close();
  }
  ...
  updateOrderStatus = (status: number, id?: string): void => {
